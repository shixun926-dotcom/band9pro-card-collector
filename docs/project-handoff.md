...
`D:\ProgramData\codex\Project0905\band9pro-card-collector\release\com.lovepower.starcard.debug.0.1.0.rpk`
...
`D:\ProgramData\codex\Project0905\band9pro-card-collector\vela`
...
```powershell
cd D:\ProgramData\codex\Project0905\band9pro-card-collector\vela
```
...
```text
D:\ProgramData\codex\Project0905\band9pro-card-collector\vela\dist\com.lovepower.starcard.debug.0.1.0.rpk
```

验证核心逻辑和体积限制：

```powershell
cd D:\ProgramData\codex\Project0905\band9pro-card-collector
npm run verify
```
...
如果应用卡住，应返回表盘或重启手环，然后通过 AstroBox 卸载应用。应用不会在开机时自动运行，也不会常驻后台。
